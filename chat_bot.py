...
from r2g.report_generate import reportGen, reportGenOtter
...
class gpt_bot(base_bot):
    ...
    def save_classifier_result(self, report, path):
        self.save_count += 1
        print(self.save_count)
        if os.path.exists(path): return
        print(f"saving classifier results in {path}")
        with open(path, 'w') as f:
            f.write(report.replace("Network A:", "").strip())

    def report_cxr_en(self,img_path, mode:str='run', save_classifier_result=False):
        img1,img2=getJFImg(img_path,self.imgcfg)
        if isinstance(self.reporter, Generator):
            text_report=self.reporter.report(img1)[0]
        elif isinstance(self.reporter, GeneratorOtter):
            text_report=self.reporter.report(img_path)
        else:
            raise ValueError
        ...
        if save_classifier_result:
            self.save_classifier_result(res, self.classifier_result_dir / f"{img_path.parent.name}.txt")
            return
        prompt_report_en=res+" Network B generated report: "+text_report
        awesomePrompt_en="\nRefine the report of Network B based on results from Network A. Please do not mention Network A or Network B in your response. Suppose you are a doctor writing findings for a chest x-ray report."
    ...
    def report_en(self,img_path, mode:str='run', save_classifier_result=False):
        # identify modality 
        index=self.identifier.identify(img_path)
        # call ModalitySpecificModel
        return self.report_cxr_en(img_path, mode, save_classifier_result), self.modality[0]
        if index==0:
    ...
    def chat_en(self,message: str, ref_record: str):
        ...
        if ret==None:
            ...
            knowledge=knowledge.replace("\n\n","\n")
            # needed_site=ret_website(query)
            needed_site=self.ret_local(query,mode=1)
